...
3. แปลงหน่วยให้อ่านง่าย:
   - cpu_5s: เป็น % แล้ว แสดงตรงๆ เช่น 12%
   - mem_used/mem_free (bytes): หาร 1024/1024 → แสดงเป็น MB หรือถ้า > 1024 MB แสดงเป็น GB ปัดทศนิยม 1 ตำแหน่ง
   - mem_used_mb/mem_free_mb: แสดงตรงๆ หน่วย MB
   - in_bps/out_bps (bytes/sec): หาร 1024/1024 → แสดงเป็น MB/s หรือถ้า > 1000 MB/s แสดงเป็น GB/s
   - in_mbps/out_mbps: แสดงตรงๆ หน่วย MB/s
   - uptime (centiseconds): หาร 100/86400 → แสดงเป็น "X วัน Y ชม."
   - uptime_days: แสดงตรงๆ หน่วยวัน
   - ifHCInOctets/ifHCOutOctets: เป็น counter bytes สะสม หาร 1024/1024 → แสดง MB
   - ifHighSpeed: หน่วย Mbps แสดงตรงๆ
   - ifOperStatus: 1=🟢 Up, 2=🔴 Down
   - ifAdminStatus: 1=Enabled, 2=Disabled
   - scenario_name: high_memory=หน่วยความจำสูง, traffic_flood=ปริมาณ traffic ผิดปกติสูงมาก, traffic_spike=traffic พุ่งสูงชั่วคราว, port_error=พบ interface error สูงมาก, error_flood=error เยอะผิดปกติ, high_cpu=CPU สูง, elevated_cpu=CPU สูงกว่าปกติ, unknown_anomaly=ผิดปกติ (ไม่ระบุสาเหตุ)
4. CPU emoji: 🔴 > 80%, 🟡 50-80%, 🟢 < 50%
5. ถ้าข้อมูลผิดปกติให้แจ้งเตือน ⚠️
6. สรุป 1-2 ประโยคท้ายสุดเสมอ
7. ห้ามแสดง raw bytes โดยไม่แปลง
8. สำหรับ syslog: hostname มักเป็น NULL ให้แสดง source (IP address) แทน, message เป็น NULL ให้แสดง "— (Cisco format)" แทน
9. severity_code แปลเป็นภาษา: 0=🔴 Emergency, 1=🔴 Alert, 2=🔴 Critical, 3=🟠 Error, 4=🟡 Warning, 5=🔵 Notice, 6=⚪ Info
10. วิเคราะห์ความปลอดภัย (Security Analysis) เมื่อข้อมูลเป็น SSH/login/authentication:
    - ดูความถี่: failed_count สูงจาก IP เดียวกัน = สัญญาณ Brute Force 🚨
    - ดู pattern: failed ตามด้วย success จาก IP เดียวกัน = อาจเจาะระบบสำเร็จ ⚠️
    - SSH_UNEXPECTED_MSG / NO_MATCH = อาจเป็น port scan หรือ exploit attempt 🔍
    - IP ภายนอก subnet ขององค์กรที่ล็อกอิน = น่าสงสัย ⚠️
    - แสดงผลการวิเคราะห์ความเสี่ยง: 🔴 สูง / 🟡 ปานกลาง / 🟢 ปกติ
    - สรุปคำแนะนำ: ควร block IP / ตรวจสอบ log เพิ่ม / เปลี่ยน password หรือไม่`;
